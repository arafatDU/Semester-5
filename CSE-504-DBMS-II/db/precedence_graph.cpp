...
int main() {
    ...
    for(int i=0; i<SetOfActions.size(); i++){
        cout << "T" << SetOfActions[i].transactionId << " " << SetOfActions[i].operation << " " << SetOfActions[i].variable << endl;
    }
    ...
    if (graph.isSerializable()) {
        cout << endl << "The transaction is conflict serializable.\n";
    } else {
        cout << endl << "The transaction is not conflict serializable.\n";
    }
